...
class VideoListSerializer(serializers.ModelSerializer):
    ...
    def get_thumbnail_url(self, obj):
        """Thumbnail-URL nur wenn Datei existiert, sonst Platzhalter."""
        try:
            if obj.thumbnail:
...
class VideoDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'view_count', 'rating', 'file_size', 'created_at', 'updated_at']
...
class VideoCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'cast', 'release_year', 'language', 'age_rating', 'status', 'is_featured'
        ]
        read_only_fields = ['id']
    ...
    def create(self, validated_data):
        ...
        if category_ids:
            video.categories.set(Category.objects.filter(id__in=category_ids))
        return video
...
class VideoCommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = VideoComment
        fields = ['id', 'video', 'user', 'user_name', 'user_email', 'content', 'is_edited', 'created_at', 'updated_at']
...
class VideoRatingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = VideoRating
        fields = ['id', 'video', 'user', 'user_name', 'rating', 'created_at', 'updated_at']
        read_only_fields = ['id', 'user', 'created_at', 'updated_at']

    def validate_rating(self, value):
        if value < 1 or value > 5:
            raise serializers.ValidationError("Bewertung muss zwischen 1 und 5 liegen.")
        return value
...
class VideoStreamSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Video
        fields = ['id', 'title', 'description', 'thumbnail_url', 'duration', 'formatted_duration', 'available_qualities', 'hls_streams']
