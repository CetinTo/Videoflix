...
class LegalPageViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def _get_page(self, page_type, lang):
        ...
        return LegalPage.objects.filter(page_type=page_type, language=lang, is_published=True).first()
    ...
    @extend_schema(description='Returns privacy policy page. Query: ?lang=de|en (default: de)')
    @action(detail=False, methods=['get'])
    def privacy(self, request):
        return self._legal_page_response(request, 'privacy', 'Privacy policy not found')

    @extend_schema(description='Returns imprint page. Query: ?lang=de|en (default: de)')
    @action(detail=False, methods=['get'])
    def imprint(self, request):
        return self._legal_page_response(request, 'imprint', 'Imprint not found')

    @extend_schema(description='Returns terms of service page. Query: ?lang=de|en (default: de)')
    @action(detail=False, methods=['get'])
