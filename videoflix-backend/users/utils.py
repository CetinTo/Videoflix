...
def send_activation_email(user, uid, token):
    """Send HTML activation email to user (design: Confirm your email)."""
    activation_link = _build_activation_link(uid, token)
    user_name = (user.first_name or (user.email or '').split('@')[0] or 'User').strip() or 'User'
    
    subject = 'Confirm your email'
    text_content = get_activation_email_text(activation_link, user_name)
    html_content = get_activation_email_html(activation_link, user.email, user_name)
...
def send_password_reset_email(user, uid, token):
    """Send HTML password reset email to user (design: Reset your Password)."""
