...
class LogoutView(APIView):
    ...
    @extend_schema(description='Logs out user by removing HTTP-only cookies.')
    def post(self, request):
...
class PasswordResetConfirmView(APIView):
    ...
    @extend_schema(description='Validate reset token. Returns uid and token for the frontend to use in the POST request.')
    def get(self, request, uidb64, token):
        ...
        return Response({'detail': 'Token is valid. Submit new_password and confirm_password via POST to this URL.', 'uid': uidb64, 'token': token}, status=status.HTTP_200_OK)

    @extend_schema(description='Set new password using uid and token from the reset email.')
...
def extract_registration_data(request):
    """Extract email/password from DRF request data."""
    data = request.data
    email = str(data.get('email', '')).strip()
    password = str(data.get('password', '')).strip()
    confirmed = str(data.get('confirmed_password') or data.get('confirm_password', '')).strip()
